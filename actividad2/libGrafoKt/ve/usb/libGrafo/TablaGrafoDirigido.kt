...
public class TablaGrafoDirigido(val nombreArchivo: String) {
    ...
    init {
        ...
        g = GrafoDirigido(lines[0].toInt())
        var a : List<String>
        for (lado in 3 until lines[1].toInt() + 3) {
            a = lines[lado].split('\t')
            g.agregarArco(Arco(stringToVertice[a[0]]!!, stringToVertice[a[1]]!!))
        }
    }

     /**
      * Indica si el índice si el indice [v] de un vértice pertenece al digrafo. En caso
    ...
    fun nombreVertice(v: Int) : String {
     ...
     /**
      * Retorna el digrafo asociado al grafo con vértices con nombre, indicado en el archivo
      * de entrada.
      * Tiempo de la operación: O(1).
      */
